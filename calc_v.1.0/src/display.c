...
void display(GtkButton *button, gpointer user_data) {
  ...
  if (check_expression(text, "credit")) {
    ...
    if (!error) {
      ...
      if (!error)
        sscanf(app_data->calc_data->x_input, "%Lf", &app_data->calc_data->x);
    }

    if (!error) {
      int count = parser(app_data->calc_data->input_buffer, lexem,
                         app_data->calc_data->x);
      ...
      if (graph_flag) {
        ...
        gtk_entry_set_text(GTK_ENTRY(front_data->entry2),
                           app_data->calc_data->x_input);
    ...
    free_input(calc_data);
    gtk_entry_set_text(GTK_ENTRY(front_data->entry),
                       app_data->calc_data->input_buffer);
    gtk_entry_set_text(GTK_ENTRY(front_data->entry2),
                       app_data->calc_data->x_input);
    app_data->calc_data->x = 0.0;
    app_data->calc_data->in = false;
    ...
    if (check_expression(text, "+/-")) {
      ...
      gtk_entry_set_text(GTK_ENTRY(front_data->entry),
               ...
               check_expression(text, "tan") ||
               check_expression(text, "asin") ||
               check_expression(text, "acos") ||
               check_expression(text, "atan") ||

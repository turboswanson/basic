...
void open_credit_window() {
  ...
  credit_data->m_label = GTK_WIDGET(gtk_builder_get_object(new_builder, "m_label"));
  credit_data->t_label = GTK_WIDGET(gtk_builder_get_object(new_builder, "t_label"));
  credit_data->over_label = GTK_WIDGET(gtk_builder_get_object(new_builder, "over_label"));
  credit_data->last_label = GTK_WIDGET(gtk_builder_get_object(new_builder, "last_label"));
  ...
  g_signal_connect(credit_data->loan_entry, "focus-in-event", G_CALLBACK(set_labels), credit_data);
...
void differential_calculation(double loan,double period,double rate,CreditData *credit_data) {
  ...
  for(int i = 1; i <= period; i++){
    ...
    if(i%12 == 2){
      days = 28;
    }else if(i%12 == 1 || i%12 == 3 || i%12 == 5 || i%12 == 7 || i%12 == 8 || i%12 == 10 || i%12 == 0) {
      days = 31;
...
void display_result(CreditData *credit_data,gboolean r2){
  ...
  if(r2) {
      sprintf(m_str, "The first payment: %.2lf", credit_data->monthly_payment_first);
      gtk_label_set_text(GTK_LABEL(credit_data->m_label), m_str);

      sprintf(last_str, "The last payment: %.2lf", credit_data->monthly_payment_last);

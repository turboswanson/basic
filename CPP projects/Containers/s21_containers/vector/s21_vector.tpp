...
static void shift_array_right(T* arr, int index, uint size) {
  for (int i = static_cast<int>(size); i > index; i--) {
    arr[i] = arr[i - 1];
  }
}
...
/// @brief
/// @tparam T
/// @details numeric_limits<size_type>::max() returns UINT_MAX depends on 32 or
/// 64-bit system
...
void vector<T>::erase(typename vector<T>::iterator pos) {
  ...
  if (!size_ || index == static_cast<int>(size_) || index < 0) {
